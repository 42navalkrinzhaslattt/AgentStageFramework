...
// WithVideoModel sets the AI model for video generation
func WithVideoModel(model string) AssetOption {
	return func(ag *AssetGenerator) {
		if ag.config == nil {
			ag.config = &AssetConfig{}
		}
		ag.config.VideoModel = model
	}
}
...
type Asset struct {
	ID          string                 `json:"id"`
	Type        string                 `json:"type"` // "image", "texture", "model", "audio", "video"
...
type Dimensions struct {
	...
	Duration float64 `json:"duration,omitempty"` // For video assets
	FPS      int     `json:"fps,omitempty"`      // For video assets
}
...
type VideoRequest struct {
	...
	Quality        string                 `json:"quality,omitempty"`
	MotionStrength float64                `json:"motion_strength,omitempty"` // Animation intensity
	Seed           int64                  `json:"seed,omitempty"`
...
func (ag *AssetGenerator) GenerateVideo(ctx context.Context, req *VideoRequest) (*Asset, error) {
	// Check cache first
	if ag.config != nil && ag.config.CacheEnabled {
		if cached := ag.getCachedAsset(req.Prompt, "video"); cached != nil {
			return cached, nil
		}
	}
	...
	if req.Duration == 0 {
		req.Duration = 10.0 // Default to 10 seconds
	}
	...
	// Generate video using Theta client
	videoReq := &theta_client.VideoGenerationRequest{
		Prompt:         enhancedPrompt,
		Width:          req.Width,
		Height:         req.Height,
		Duration:       req.Duration,
		FPS:            req.FPS,
		Quality:        ag.getQualityLevel(),
